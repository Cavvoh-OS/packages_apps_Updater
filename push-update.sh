...
updates_dir=/data/cavvoh_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have CcavvohOS-VERSION-DATE-TYPE-* as format"
...
# Assume CavvohOS-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.cavvoh.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.cavvoh.updater/databases/updates.db" \
